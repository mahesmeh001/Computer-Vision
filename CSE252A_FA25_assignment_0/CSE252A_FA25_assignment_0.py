...
def vectorized(img):
    ...
    image[:a, b:] = np.stack([np.zeros_like(get_channel(image[:a, b:], 2)), np.zeros_like(get_channel(image[:a, b:], 2)), get_channel(image[:a, b:], 2)], axis=2)

    #Bottom Left: (B,G,R) image
    image[a:,:b] = merge_channels(get_channel(image[a:,:b], 2), get_channel(image[a:,:b], 1), get_channel(image[a:,:b], 0))
...
# This returns different coordinates because Numpy uses row,col = y,x, but cartesian coordinates are x,y which is why there is a difference. 
...
# (5) Just place the cutout of the USA at the right position on the world map (very closely aligns with USA map California state boundary, but not perfectly)
start_x = 17
start_y = 970
...
# Given the `start_x` and `start_y` on the California image indicating the starting position (top-left corner) of the California image, 
# you need to write a function to place just the cutout of the California map at the right position on the USA map. 
...
# isoluate the california area on the big map
h, w, c = california_img.shape
roi = final_img[start_y:start_y+h, start_x:start_x+w, :] # acts as a refernence
